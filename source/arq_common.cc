...
void cl_arq_controller::load_configuration(int configuration)
{
	...
	message_transmission_time_ms=ceil((1000.0*telecom_system->data_container.Nsymb*telecom_system->data_container.Nofdm)/(float)(telecom_system->frequency_interpolation_rate*(telecom_system->bandwidth/telecom_system->ofdm.Nc)*telecom_system->ofdm.Nfft));
	set_ack_timeout((2*(data_batch_size+1.5+ack_batch_size+1.5))*message_transmission_time_ms);
...
void cl_arq_controller::ptt_on()
{
	cl_timer delay;
	delay.start();
	while(delay.get_elapsed_time_ms()<message_transmission_time_ms*0.5);
