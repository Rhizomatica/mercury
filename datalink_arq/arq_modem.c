...
int arq_modem_preferred_rx_mode(const arq_session_t *sess)
{
     ...
     * arrives on the fast DATAC14; data RX is selected separately via
     * peer_tx_mode.  Before connect (LISTENING/CONNECTING) the CALL/ACCEPT
...
int arq_modem_preferred_tx_mode(const arq_session_t *sess)
{
    ...
    if (sess->conn_state == ARQ_CONN_CONNECTED)
    {
        ...
        return ARQ_INSESSION_MODE;   /* in-session control on DATAC14 */
    }
