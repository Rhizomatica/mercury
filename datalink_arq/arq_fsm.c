...
static void send_ctrl_frame(arq_session_t *sess, arq_subtype_t subtype)
{
     ...
     * carry no seq.  All ride the compact in-session control mode. */
...
static void send_ack(arq_session_t *sess, uint8_t ack_delay_raw)
{
    ...
    {
        /* The connect-confirmation ACK: the peer is still in the connect
         * phase (ACCEPTING) listening on the DATAC16 connect mode, so this
         * one ACK must ride DATAC16 with the full header.  Every later ACK is
         * compact on DATAC14. */
