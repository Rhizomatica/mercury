...
/* Dual control plane:
 *  - ARQ_CONNECT_MODE (DATAC16, robust, 14 B): the rare connect handshake
 *    (CALL/ACCEPT/CQ) that must carry callsigns — robustness over airtime
 *    since it happens once per link.
 *  - ARQ_INSESSION_MODE (DATAC18, rate-0.30, ~1.54 s, 4 B): the frequent
 *    per-turnaround frames (ACK/TURN/KEEPALIVE/DISCONNECT/MODE) using the
 *    compact 4-byte header — airtime is paid every turnaround.  DATAC18 is a
 *    Mercury-custom mode (additive; does NOT redefine upstream DATAC14).
 * ARQ_CONTROL_MODE keeps the legacy name = the connect mode. */
#define ARQ_CONNECT_MODE     FREEDV_MODE_DATAC16
#define ARQ_INSESSION_MODE   FREEDV_MODE_DATAC18
...
/** Compact 4-byte in-session control frame (DATAC18 carrier):
...
/** True for either control-plane mode (DATAC16 connect / DATAC18 in-session) —
