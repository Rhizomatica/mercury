...
typedef struct
{
    ...
    /* After CONNECTED, on stations with [crypto] mode != off: VARA's
     * "ENCRYPTED LINK" or "UNENCRYPTED LINK". */
    void (*send_encryption)(bool encrypted);
} arq_tnc_callbacks_t;
...
void arq_tnc_send_encryption(bool encrypted);
