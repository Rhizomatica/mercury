...
typedef enum { search, trial, synced } State;
...
struct OFDM {
  ...
  int samplesperframe;
  int nrxbufhistory; /* extra storage at start of rxbuf to allow us to step back
                        in time */
  int nrxbufmin; /* min number of samples we need in rxbuf to process a modem
                    frame */
