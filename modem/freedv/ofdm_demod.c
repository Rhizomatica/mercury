...
static const char *statemode[] = {"search", "trial", "synced"};

void opt_help() {
  fprintf(stderr, "\nusage: %s [options]\n\n", progname);
  fprintf(stderr,
          "  Default output file format is one byte per bit hard decision\n\n");
  fprintf(stderr, "  --in          filename   Name of InputModemRawFile\n");
  fprintf(stderr,
          "  --out         filename   Name of OutputOneCharPerBitFile\n");
  fprintf(stderr, "  --log         filename   Octave log file for testing\n");
  fprintf(
      stderr,
      "  --mode       modeName    Predefined mode e.g. 700D|2020|datac1 etc\n");
  fprintf(
      stderr,
      "  --nc          [17..62]   Number of Carriers (17 default, 62 max)\n");
  fprintf(stderr, "  --np                     Number of packets\n");
  fprintf(
      stderr,
      "  --ns           Nframes   One pilot every ns symbols (8 default)\n");
  fprintf(stderr,
          "  --tcp            Nsecs   Cyclic Prefix Duration (.002 default)\n");
  fprintf(stderr,
          "  --ts             Nsecs   Symbol Duration (.018 default)\n");
  fprintf(stderr,
          "  --bandwidth      [0|1]   Select phase est bw mode AUTO low or "
          "high (0) or LOCKED high (1) (default 0)\n");
  fprintf(stderr,
          "                           Must also specify --ldpc option\n");
  fprintf(stderr,
          "  --tx_freq         freq   Set modulation TX centre Frequency "
          "(1500.0 default)\n");
  fprintf(stderr,
          "  --rx_freq         freq   Set modulation RX centre Frequency "
          "(1500.0 default)\n");
  fprintf(stderr,
          "  --verbose      [1|2|3]   Verbose output level to stderr (default "
          "off)\n");
  fprintf(stderr,
          "  --testframes             Receive test frames and count errors\n");
  fprintf(stderr, "  --ldpc                   Run LDPC decoder\n");
  fprintf(stderr, "\n");
  fprintf(stderr,
          "  --start_secs      secs   Number of seconds delay before we start "
          "to demod\n");
  fprintf(stderr,
          "  --len_secs        secs   Number of seconds to run demod\n");
  fprintf(stderr,
          "  --skip_secs   timeSecs   At timeSecs introduce a large timing "
          "error by skipping half a frame of samples\n");
  fprintf(stderr,
          "  --packetsperburst p      use burst mode; number of packets we "
          "expect per burst\n");
  fprintf(stderr, "\n");
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = optparse_long(&options, longopts, NULL)) != -1) {
    switch (opt) {
      ...
      case 'v':
        ...
        if (verbose < 0 || verbose > 3) verbose = 0;
        break;
      case 'x':
  ...
  /* Get a copy of the actual modem config (ofdm_create() fills in more
   * parameters) */
  ofdm_config = ofdm_get_config_param(ofdm);

  int ofdm_bitsperframe = ofdm_get_bits_per_frame(ofdm);
  int ofdm_rowsperframe =
      ofdm_bitsperframe / (ofdm_config->nc * ofdm_config->bps);
  ...
  /* zero out the log arrays in case we don't run for NFRAMES and fill them with
   * data */
  ...
  int Terrs, Tbits, Terrs2, Tbits2, Terrs_coded, Tbits_coded, frame_count,
      packet_count, Ndiscard;
  Terrs = Tbits = Terrs2 = Tbits2 = Terrs_coded = Tbits_coded = frame_count =
      packet_count = 0;
  int Nerrs_raw = 0;
  int Nerrs_coded = 0;
  int Ncoded;
