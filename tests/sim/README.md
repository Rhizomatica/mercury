...
undelivered.  That is the turn-handover bench, and the integrity check matters
most here: it caught a two-way-only corruption (a torn-down session resurrected
by the peer's late DATA) that no one-way run could show.
